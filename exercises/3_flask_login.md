...
4. Add custom feedback for unathorised access to pages
...
The login manager now needs to be initialised for Flask in the `create_app()` function. It is currently commented out,
remove the comments. We will also add an additional line to tell login manager which view to load if the user accesses a
...
example we can modify our User class like this:
...
### 8. Modify the homepage to check if a user is logged in
Modify the route for `main.index` to check if the user is logged in and display their name if so.
